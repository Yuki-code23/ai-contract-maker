...
import { Search, Filter, Calendar, ChevronDown, Download, Copy, Trash2, Edit, FileText, Plus } from 'lucide-react';
...
export default function BillingsPage() {
    ...
    const loadBillings = async () => {
        try {
            setLoading(true);
            const data = await getBillings('payable'); // Fetch payables
            setBillings(data);
        } catch (error) {
    ...
    const handleDelete = async (id: number) => {
        if (!confirm('この支払通知書を削除してもよろしいですか？')) return;
        try {
    ...
    const handleEdit = (id: number) => {
        router.push(`/payments/${id}/edit?editId=${id}`);
    };

    const handleDuplicate = (id: number) => {
        router.push(`/payments/new?duplicateId=${id}`);
    };
        ...
        <div className="flex h-screen bg-gray-50 dark:bg-[#131314] text-gray-900 dark:text-gray-100">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto p-8">
                    {/* Dashboard Summary - Hidden for now or adapt for Payments? */}
                    {/* <InvoiceDashboard billings={billings} onSelectFilter={handleDashboardFilter} /> */}

                    <div className="flex items-center gap-4 mb-6">
                        <Link href="/payments/new" className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors font-medium shadow-sm">
                                    ...
                                    filteredBillings.map((billing) => (
                                        <tr key={billing.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group ${(billing.payment_deadline && new Date(billing.payment_deadline) < new Date() && billing.status !== 'Paid') ? 'bg-red-100 dark:bg-red-900/30' : ''}`}>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <Link href={`/payments/${billing.id}`} className="block">
                                                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                                        {(billing.client_info as any)?.name || billing.contractPartyB || '名無しの支払先'}
                                                    </div>
