...
export default function BillingDetailPage({ params }: { params: { id: string } }) {
    ...
    const labels = {
        title: isPayable ? '支払通知書' : '請求書',
        number: isPayable ? '支払番号' : '請求番号',
        date: isPayable ? '支払日' : '支払期限',
        action: isPayable ? '支払通知書PDFダウンロード' : '請求書PDFダウンロード',
    };
        ...
        <div className="flex h-screen bg-gray-50 dark:bg-[#131314] text-gray-900 dark:text-gray-100">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                <header className="px-8 py-6 border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-[#131314] flex items-center gap-4">
                    <BackButton />
                    <h1 className="text-2xl font-bold">{labels.title}詳細 #{billing.id}</h1>
                </header>
                ...
                <main className="flex-1 overflow-y-auto p-8">
                    <div className="max-w-4xl mx-auto space-y-6">
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4 rounded-lg text-sm">
                            <h3 className="font-bold text-blue-800 dark:text-blue-300 mb-2">{labels.title}ステータスの定義</h3>
                            <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 text-blue-700 dark:text-blue-400">
                                <li><strong>Planned (予定)</strong>: {isPayable ? '支払' : '請求'}の準備段階です。</li>
                                <li><strong>Approved (承認済)</strong>: 内容が確定し、{isPayable ? '通知' : '送付'}可能な状態です。</li>
                                <li><strong>Sent (送付済)</strong>: 顧客へ{labels.title}を送付した状態です。</li>
                                <li><strong>Paid (入金済)</strong>: {isPayable ? '支払' : '入金'}が完了した状態です。</li>
                            </ul>
                        </div>
                        ...
                        <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
                            <div className="flex justify-between items-start mb-8">
                                <div>
                                    ...
                                    <div className="text-sm text-gray-600 dark:text-gray-400">
                                        <p>発行日: {billing.issue_date || '-'}</p>
                                        <p>{labels.date}: {billing.payment_deadline || '-'}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                            ...
                            <div className="flex justify-end pt-4 border-t border-gray-200 dark:border-gray-700">
                                {bankInfo && !isPayable && (
                                    <div className="mr-auto text-sm bg-gray-50 dark:bg-gray-800/50 p-4 rounded text-gray-600 dark:text-gray-400">
                                        <p className="font-bold mb-1">お振込先</p>
                                        <p>{bankInfo.bank_name} {bankInfo.branch_name}</p>
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-6 py-3 bg-gray-900 dark:bg-gray-700 text-white rounded-md hover:bg-gray-800 dark:hover:bg-gray-600 transition-colors disabled:opacity-50 h-fit"
                                >
                                    {generatingPdf ? '生成中...' : labels.action}
                                </button>
                            </div>
                        </div>
                    </div>
